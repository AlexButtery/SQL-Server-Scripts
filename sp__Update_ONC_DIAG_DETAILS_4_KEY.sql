...
/****** Object:  StoredProcedure [dbo].[sp__Update_ONC_DIAG_DETAILS_4_KEY]    Script Date: 02/23/2011 08:22:32 ******/
SET ANSI_NULLS ON
GO
...
CREATE Procedure [dbo].[sp__Update_ONC_DIAG_DETAILS_4_KEY]
AS
-------------------------------------------------------------------------------
-- This stored Procedure is used to update the ONC_DIAG_DETAILS_4_KEY        --
...
BEGIN
	...
	DECLARE @DiagId                   INT;
	DECLARE @OncDiagDetails4Key       INT;
	...
	FOR SELECT A.PATIENT_VISIT_ONC_ID, A.PATIENT_DIAGNOSIS_ONC_ID, B.ONC_DIAG_DETAILS_4_KEY
          FROM IRXTST..ARCHIVE.TEMP_UPD_TREAT_REGMN_CHG_IND A INNER JOIN IRXTST..IRX.FACT2_PATIENT_DIAG_ONC B 
	...
	OPEN UpdateList;
	FETCH NEXT FROM UpdateList into @PatientVisitId, @DiagId, @OncDiagDetails4Key
	...
	BEGIN
		...
		BEGIN
			UPDATE cdomii_dy.dbo.FACT2_PATIENT_DIAG_ONC
			  SET ONC_DIAG_DETAILS_4_KEY = @OncDiagDetails4Key
